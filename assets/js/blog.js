/**
 * blog.js - Blog post search and tag filtering
 */

(function() {
  const COLLECTIONS_INDEX = '/assets/search/blog.json';
  let allItems = [];
  let itemsContainer = null;

  function init() {
    itemsContainer = getElement('blog-items');
    if (!itemsContainer) {
    ...
    fetch(COLLECTIONS_INDEX)
      ...
      .catch(error => console.error('Error loading blog index:', error));
  }
  ...
  function renderItems(items) {
    ...
    if (items.length === 0) {
      itemsContainer.innerHTML = '<div class="no-results"><p>No posts found. Try adjusting your search or filters.</p></div>';
      return;
    }
  ...
  function updateFilterInfo(query, tag, resultCount, totalCount) {
    const filterInfoEl = getElement('blog-filter-info');
    ...
    if (query || tag) {
      ...
      info += ` — ${resultCount} of ${totalCount} posts`;
    } else {
      info = `${totalCount} posts`;
    }
